...
func TestDependencySatisfied(t *testing.T) {
	if !dependencySatisfied(nil, 1.0, false, AssertionKindEmpirical) {
		t.Error("expected true for 1.0 false")
	}
	if dependencySatisfied(nil, 0.5, false, AssertionKindEmpirical) {
		t.Error("expected false for 0.5 false")
	}
	if !dependencySatisfied(nil, 0.5, true, AssertionKindEmpirical) {
